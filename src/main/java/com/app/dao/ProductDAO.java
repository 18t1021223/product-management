...
import com.app.Mapping.ProductMapping;
...
public class ProductDAO {
    ...
    public boolean save(Product product) {
        try {
            ...
            String sql = "insert into product(product_id , product_name , product_price , product_cost , product_image , product_unit) values(?,?,?,?,?,?)";
    ...
    public boolean update(Product product) {
        try {
            ConnectSql.setAutocommit(connection, false);
            String sql = "update product set product_name = ? , product_price = ? , product_cost = ?, product_image = ? , product_unit = ? ,modifiedDate = ? where product_id = ? ";
            PreparedStatement statement = connection.prepareStatement(sql);
            ...
            JOptionPane.showMessageDialog(null, exception.getMessage(), "Lỗi cập nhật sản phẩm", JOptionPane.ERROR_MESSAGE);
            ConnectSql.rollBack(connection);
        } finally {
            ConnectSql.setAutocommit(connection, true);
        }
